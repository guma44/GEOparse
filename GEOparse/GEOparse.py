...
def get_GEO(geo=None, filepath=None, destdir="./", how='full', annotate_gpl=False, geotype=None, include_data=False, silent=False):
    ...
    if (geo is not None and filepath is not None):
        raise Exception(
            "You can specify filename or GEO accession - not both!")

    if filepath is None:
        filepath, geotype = get_GEO_file(
            geo, destdir=destdir, how=how, annotate_gpl=annotate_gpl, include_data=include_data)
    else:
        if geotype is None:
    ...
    if geotype.upper() == "GSM":
        ...
        raise ValueError(
            "Unknown GEO type: %s. Available types: GSM, GSE, GPL and GDS." % geotype.upper())
...
def get_GEO_file(geo, destdir=None, annotate_gpl=False, how="full",
    ...
    if geotype == "GDS":
        ...
        filepath = path.join(
            tmpdir, "{record}_family.soft.gz".format(record=geo))
    elif geotype == "GSM":
        ...
        if annotate_gpl:
            ...
            filepath = path.join(
                tmpdir, "{record}.annot.gz".format(record=geo))
            if not path.isfile(filepath):
                try:
                    ...
                    stderr.write(
                        "Annotations for %s are not available, trying submitter GPL\n" % geo)
            else:
            ...
            for is_new_entry, group in groupper:
                if is_new_entry:
                    ...
                    if entry_type == "SERIES":
                        ...
                        gses[entry_name] = GSE(
                            name=entry_name, metadata=gse_metadata)
